...
export default function Index() {
  ...
  return (
    <div className="text-white/90 bg-stone-900 p-5">
          ...
          <li key={id} className="flex flex-col bg-white/5 rounded-xl">
            <ul className="flex gap-5 p-5">
              {attributes?.products?.data.map(({ id, attributes }) => (
                <li key={id} className="p-20 rounded-xl bg-white/5" />
              ))}
            </ul>
            <div className="py-2 px-4 flex justify-between items-center bg-white/5 rounded-b-xl">
              <span className="font-semibold text-3xl">{attributes?.Name}</span>
              <Link
                to={`/category/${id}`}
                className=" bg-white/5 hover:bg-white/10 py-3 px-5 rounded-xl transition-colors ease-in-out duration-200 text-base uppercase font-black tracking-widest"
              >Details</Link>
            </div>
          </li>
        ))}
